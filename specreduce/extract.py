...
class BoxcarExtract(SpecreduceOperation):
    ...
    def __call__(self, img, trace_object):
        ...
        for i in range(0, len(trace_line)):
            ...
            y = np.append(np.arange(itrace_line-self.apwidth-self.skysep-self.skywidth,
                                    itrace_line-self.apwidth-self.skysep),
                          np.arange(itrace_line+self.apwidth+self.skysep+1,
    ...
    def get_checkplot(self):
        ...
        plt.fill_between(np.arange(len(trace_line)), trace_line + self. apwidth,
                         trace_line-self.apwidth, color='C0', alpha=0.5)
        plt.fill_between(np.arange(len(trace_line)), trace_line + self.apwidth + self.skysep,
                         trace_line + self. apwidth + self.skysep + self.skywidth,
                         color='C1', alpha=0.5)
        plt.fill_between(np.arange(len(trace_line)), trace_line - self. apwidth - self.skysep,
                         trace_line - self. apwidth - self.skysep - self.skywidth,
                         color='C1', alpha=0.5)
        plt.ylim(np.min(trace_line - (self. apwidth + self.skysep + self.skywidth)*2),
                 np.max(trace_line + (self. apwidth + self.skysep + self.skywidth)*2))
